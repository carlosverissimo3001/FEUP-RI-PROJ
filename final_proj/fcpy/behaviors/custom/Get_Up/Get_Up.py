...
class Get_Up():
    ...
    def execute(self,reset):
        ...
        if self.state == 0: # State 0: go to pose "Zero"
            ...
            if (r.behavior.execute_sub_behavior("Zero",None) and len(self.gyro_queue) == self.STABILITY_THRESHOLD and all(g < 10 for g in self.gyro_queue)) or self.watchdog > 100:

                # determine how to get up
                if r.acc[0] < -4 and abs(r.acc[1]) < 2 and abs(r.acc[2]) < 3:
                    r.behavior.execute_sub_behavior("Get_Up_Front", True) # reset behavior
                    self.state = 1
                elif r.acc[0] > 4 and abs(r.acc[1]) < 2 and abs(r.acc[2]) < 3:
                    r.behavior.execute_sub_behavior("Get_Up_Back", True) # reset behavior
                    self.state = 2
                elif r.acc[2] > 8: # fail-safe if vision is not up to date: if pose is 'Zero' and torso is upright, the robot is already up
                    return True
                else:
                    r.behavior.execute_sub_behavior("Flip", True) # reset behavior
                    self.state = 3

        elif self.state == 1:
            if r.behavior.execute_sub_behavior("Get_Up_Front", False):
                return True
        elif self.state == 2:
            if r.behavior.execute_sub_behavior("Get_Up_Back", False):
                return True
        elif self.state == 3:
            if r.behavior.execute_sub_behavior("Flip", False):
                self.reset()
