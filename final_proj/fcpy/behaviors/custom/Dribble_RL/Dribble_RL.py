...
import pickle
from behaviors.custom.Dribble_RL.Env import Env
...
class Dribble_RL():
    ...
    def __init__(self, world) -> None:
        ...
        self.auto_head = True
        self.env = Env(self.world, 0.9 if world.robot.type == 3 else 1.2)

        with open(U.get_active_directory([
            "/behaviors/custom/Dribble_RL/dribble_R0_29_157286400.pkl",
            "/behaviors/custom/Dribble_RL/dribble_R1_02_445644800.pkl",
            "/behaviors/custom/Dribble_RL/dribble_R2_01_170393600.pkl",
            "/behaviors/custom/Dribble_RL/dribble_R3_07_15M.pkl",
            "/behaviors/custom/Dribble_RL/dribble_R4_00_169M.pkl"
    ...
    def define_approach_orientation(self):
        ...
        cad = U.vector_angle(b - me) # current approach direction

        a1 = U.normalize_deg(a1)
        a2 = U.normalize_deg(a2)
        ...
        a1_diff = abs(U.normalize_deg(a1 - cad))
        a2_diff = abs(U.normalize_deg(a2 - cad))
    ...
    def execute(self, reset, orientation, is_orientation_absolute, speed=1, stop=False):
        ...
        if reset:
            self.phase = 0
            if r.behavior.previous_behavior == "Push_RL" and 0<b_rel[0]<0.25 and abs(b_rel[1])<0.07:
                self.phase = 1
                reset_dribble = True

        if self.phase == 0: # walk to ball
            reset_walk = reset and r.behavior.previous_behavior != "Walk_RL3" and r.behavior.previous_behavior != "Push_RL" # reset walk if it wasn't the previous behavior
            ...
            if self.approach_orientation is not None:
                next_pos, next_ori, dist_to_final_target = r.path_manager.get_path_to_ball(x_ori=self.approach_orientation, 
                    x_dev=-0.24, torso_ori=self.approach_orientation, safety_margin=0.4)
            ...
            #------------------------ 2B. A better approach orientation is not needed but the robot cannot see the ball 
                ...
                abs_ori = U.vector_angle( b - me )
                r.behavior.execute_sub_behavior("Walk_RL3", reset_walk, b, True, abs_ori, True, None) # target, is_target_abs, ori, is_ori_abs, distance
                
            #------------------------ 2C. A better approach orientation is not needed and the robot can see the ball
            else: # walk to relative target   
                if b_rel[0]<0.25 and b_rel[0]>0.18 and abs(b_rel[1])<0.05 and w.ball_is_visible: # ready to start dribble
                    self.phase += 1
                    reset_dribble = True
                else:
                    rel_target = b_rel+(-0.23,0)    # relative target is a circle (center: ball, radius:0.23m)
                    rel_ori = U.vector_angle(b_rel) # relative orientation of ball, NOT the target!
                    dist = max(0.08, np.linalg.norm(rel_target)*0.7) # slow approach
                    r.behavior.execute_sub_behavior("Walk_RL3", reset_walk, rel_target, False, rel_ori, False, dist) # target, is_target_abs, ori, is_ori_abs, distance

            if stop:
                return True

        if self.phase == 1 and (stop or b_dist > 0.5): # go back to walking
            self.phase += 1
            ...
            if orientation is None:
                ...
                self.env.dribble_rel_orientation = r.path_manager.get_dribble_path(optional_2d_target=dribble_target)[1]
            elif is_orientation_absolute:
                self.env.dribble_rel_orientation = U.normalize_deg( orientation - r.imu_torso_orientation )
            else:
    ...
    def is_ready(self):
        ''' Returns True if Dribble Behavior is ready to start under current game/robot conditions '''
        return True
