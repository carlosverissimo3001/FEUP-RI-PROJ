...
class Env():
    ...
    def observe(self, init=False, virtual_ball=False):
        ...
        if w.vision_is_up_to_date:
            ...
            internal_ori_diff =  np.clip( U.normalize_deg( self.dribble_rel_orientation - self.internal_rel_orientation ), -MAX_ROTATION_DIFF, MAX_ROTATION_DIFF)
            self.internal_rel_orientation = np.clip(U.normalize_deg( self.internal_rel_orientation + internal_ori_diff ), -MAX_ROTATION_DIST, MAX_ROTATION_DIST)
