...
class Env_LL():
    def __init__(self, world : World, step_width) -> None:
    ...
    def observe(self, init=False):
        ...
        self.obs[74] = U.deg_cos(rel_HL_target) # x
        self.obs[75] = U.deg_sin(rel_HL_target) # y

        # -------------------------------------------- Closest Opponent

        self.obs[76] = 2 # no one near
        self.obs[77] = 2

        opps_dist = [(o.state_horizontal_dist if (o.state_last_update > 0 and w.time_local_ms - o.state_last_update < 250) else 10) for o in w.opponents]
        closest_opp_idx = np.argmin(opps_dist)
        o = w.opponents[closest_opp_idx]
        if opps_dist[closest_opp_idx] < 1: # opp is visible and at < 1 m
            # Get 2D average position of visible body parts relative to torso
            # give more importance to closer body parts
            ...
            for pos in o.body_parts_cart_rel_pos.values():
                ...
                weight = math.pow(1e6, -np.linalg.norm(bp_rel_torso_2d))
                body_parts_rel_torso_2d_avg += weight * bp_rel_torso_2d
                weight_sum += weight
            body_parts_rel_torso_2d_avg /= weight_sum # average

            self.obs[76] = body_parts_rel_torso_2d_avg[0] # x
            self.obs[77] = body_parts_rel_torso_2d_avg[1] # y
    ...
    def execute(self, action):
        ...
        self.act = 0.80 * self.act + 0.20 * action * 0.9 * self.push_speed
