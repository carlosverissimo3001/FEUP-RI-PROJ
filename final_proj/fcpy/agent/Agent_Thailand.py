...
class Agent(Base_Agent):
    ...
    def __init__(self, host:str, agent_port:int, monitor_port:int, unum:int,
        ...
        self.TIMEOUT = 3000 # timeout in us
        ...
        # Special skills
        self.SS_GET_UP = 4  # get up
        self.SS_BEAM = 5    # beam to formation
        self.SS_IDLE = 6

        self.INTENTIONS_NAMES = {None:"", self.I_MOVE: "move", self.I_DRIBBLE: "dribble", self.I_PUSH: "push", self.I_KICK: "kick", self.I_GETUP: "get_up", self.I_IDLE: "idle", self.I_ACTIVE_BEAM: "active_beam", self.I_PASSIVE_BEAM: "passive_beam"}
        self.SKILLS_NAMES = {None:"", self.S_WALK: "walk", self.S_DRIBBLE: "dribble", self.S_WALK_KICK: "walk_kick", self.S_DRIBBLE_KICK: "dribble_kick", self.SS_GET_UP: "get_up", self.SS_BEAM: "beam", self.SS_IDLE: "idle"}
    ...
    def think_and_send(self):
        ...
        if PM == w.M_GAME_OVER:
            ...
            if ball_dist > 1:
                ...
                if self.sk == self.S_WALK_KICK: # commit to kick, do not change intention or skill
                    ...
                    # Get closest standing opponent
                    opp_d = min(np.linalg.norm(p.state_abs_pos[:2]-ball_2d) if (w.time_local_ms - p.state_last_update <= 600) else 100 for p in w.opponents)
                    
                    if ball_2d[0] > 14.5 and abs(ball_2d[1]) < 1.6: 
                        self.it = self.I_PUSH # push if near opp goal
                        self.is_push_1v1 = False # careful approach
                    elif ball_2d[0] < -14.5 and np.linalg.norm(np.abs(ball_2d)-(15.02,  1.07)) < 0.5:
                        self.it = self.I_PUSH # push if near our goal posts
                        self.is_push_1v1 = False # careful approach
                    elif opp_d < ball_dist+0.4:
                        self.it = self.I_PUSH # push if opponent is dangerous
                        self.is_push_1v1 = True # fast approach, guided push
                    else:
                        #best_kick_score = max(best_short_kick[2], best_long_kick[2])

                        if best_long_kick[2] > 0.4 and is_long_to_goal and opp_d > 1.5: # do not allow passing
                            self.it = self.I_KICK
                        else:
                            self.it = self.I_DRIBBLE
     
                else: # dribble or dribble_kick
                    self.it = self.I_DRIBBLE # never kick
            else:
                raise ValueError(f'Unexpected play mode ID: {PM}')
        ...
        if self.it == self.I_KICK:
            ...
            elif self.sk != self.S_WALK_KICK and self.sk != self.S_DRIBBLE:
