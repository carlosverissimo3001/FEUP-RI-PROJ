...
# Original joint information
# hj1/hj2      Head horizontal:  right(-120)  / left(+120)      Head vertical :  down(-45)    / up(+45)
# llj1/rlj1    LLeg twist     :  fwd/out(-90) / reverse(+1)     RLeg twist    :  fwd/out(-90) / reverse(+1)
# llj2/rlj2  * LLeg side      :  in(-25)      / out(+45)        RLeg side     :  out(-45)     / in(+25)     
# llj3/rlj3    LLeg front     :  back(-25)    / front(+100)     RLeg front    :  back(-25)    / front(+100)
# llj4/rlj4    LKnee          :  back(-130)   / front(+1)       RKnee         :  back(-130)   / front(+1) 
# llj5/rlj5    LFoot vertical :  down(-45)    / up(+75)         RFoot         :  down(-45)    / up(+75)
# llj6/rlj6  * LFoot twist    :  in(-45)      / out(+25)        RFoot twist   :  out(-25)     / in(+45)     
# laj1/raj1    LArm front     :  down(-120)   / up(+120)        RArm front    :  down(-120)   / up(+120)
# laj2/raj2  * LArm side      :  in(-1)       / out(+95)        RArm side     :  out(-95)     / in(+1)      
# laj3/raj3  * LArm twist     :  out(-120)    / in(+120)        RArm twist    :  in(-120)     / out(+120)   
# laj4/raj4  * LElbow         :  up(-90)      / down(+1)        RElbow        :  down(-1)     / up(+90)     
# llj7/rlj7    Ltoe           :  down(-1)     / up(+70)         Rtoe          :  down(-1)     / up(+70)
# inverted joints: 'rlj2','rlj6','raj2','laj3','laj4'
...
class World_Parser():
    def __init__(self, world:World, hear_callback) -> None:
        ...
        self.LEFT_PLAY_MODE_TO_ID = {"KickOff_Left":World.M_OUR_KICKOFF, "KickIn_Left":World.M_OUR_KICK_IN, "corner_kick_left":World.M_OUR_CORNER_KICK,
                                    ...
                                    "BeforeKickOff": World.M_BEFORE_KICK_OFF, "GameOver": World.M_GAME_OVER, "PlayOn": World.M_PLAY_ON }
            ...
            if tag==b'time':
                ...
                while True:
                    ...
                    if tag==b'G1R' or tag==b'G2R' or tag==b'G1L' or tag==b'G2L':
                        ...
                        self.world.ball_rel_head_cart_pos = U.deg_sph2cart(self.world.ball_rel_head_sph_pos)
                        ...
                        while True:
                            ...
                            if tag==b'team':
                                ...
                                if self.world.team_name_opponent is None and not is_teammate: #register opponent team name
                                    self.world.team_name_opponent = self.world.team_name
                            elif tag==b'id':
                                ...
                                if is_teammate: 
                                    ...
                                    self.world.teammates[player_id-1].body_parts_cart_rel_pos[tag_str] = U.deg_sph2cart((c1,c2,c3))
                                else:
                                    self.world.opponents[player_id-1].body_parts_sph_rel_pos[tag_str] = (c1,c2,c3)
                                    self.world.opponents[player_id-1].body_parts_cart_rel_pos[tag_str] = U.deg_sph2cart((c1,c2,c3))
                            else:
