...
class Server_Comm():
    ...
    def __init__(self, host:str, agent_port:int, monitor_port:int, unum:int, robot_type:int, team_name:str,
        ...
        #Monitor socket is shared by all agents
    ...
    def _receive_async(self, other_players, first_pass) -> None:
        ...
        if first_pass: print("Async agent",self.unum,"initalization", end="", flush=True)
    ...
    def receive(self):
        ...
        if  i>1: self.world.log(f"Server_Comm.py: The agent lost {i} consecutive packets! Is syncmode enabled?")
        self.world.update()
    ...
    def unofficial_kill_player(self, unum : int, team_side_is_left : bool) -> None:
        ...
        '''
        msg = f"(kill (unum {unum}) (team {'Left' if team_side_is_left else 'Right'}))"
        self.monitor_socket.send( (len(msg)).to_bytes(4,byteorder='big') + msg )

    def close(self):
        ''' Close agent port, but not the monitor port '''
        self.socket.close()
