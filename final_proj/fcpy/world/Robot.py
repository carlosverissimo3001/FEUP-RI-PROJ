from world.Inverse_Kinematics import Inverse_Kinematics
from behaviors.Behavior import Behavior
from math import atan, atan2, pi, sqrt, tan
from world.Joint_Info import Joint_Info
from world.Body_Part import Body_Part
from utils.Matrix_3x3 import Matrix_3x3
...
class Robot():
    ...
    def __init__(self, unum:int, robot_type:int) -> None:
        ...
        self.loc_head_position_history = deque(maxlen=40)# Absolute head position history (queue with up to 40 old positions at intervals of 0.06s, where index 0 is the previous position)
        ...
        self.J_LARM_FRONT = 14
        self.J_RARM_FRONT = 15
        self.J_LARM_SIDE = 16
        self.J_RARM_SIDE = 17
        self.J_LARM_TWIST = 18
        self.J_RARM_TWIST = 19
        self.J_LELBOW = 20
        self.J_RELBOW = 21
        self.J_LTOE = 22
        self.J_RTOE = 23
        ...
        dir = U.get_active_directory("/robots/")
    ...
    def initialize_dependent_objects(self, world, scom):
        ''' Initialize dependent objects after creating World and Robot objects '''
        from agent.commons.Path_Manager import Path_Manager
        self.inv_kinematics = Inverse_Kinematics(world.robot)
        self.behavior = Behavior(world, scom)
        self.path_manager = Path_Manager(world, True) # enable path manager drawings (does not override the draw enable property)
    ...
    def get_head_abs_vel(self, history_steps:int):
        ...
        Examples
        --------
        get_head_abs_vel(1) is equivalent to (current abs pos - last abs pos)      / 0.06
        get_head_abs_vel(2) is equivalent to (current abs pos - abs pos 0.12s ago) / 0.12
        get_head_abs_vel(3) is equivalent to (current abs pos - abs pos 0.18s ago) / 0.18
        '''
