...
- class Walk_Forward: implements an OpenAI custom gym
- class Train:  implements algorithms to train a new model or test an existing model
'''


class Run_Forward(gym.Env):
    ...
    def observe(self, init=False):
        ...
        self.obs[12:18] = r.frp.get('lf', (
        0, 0, 0, 0, 0, 0))  # left foot: relative point of origin (p) and force vector (f) -> (px,py,pz,fx,fy,fz)*
        self.obs[18:24] = r.frp.get('rf', (
        0, 0, 0, 0, 0, 0))  # right foot: relative point of origin (p) and force vector (f) -> (px,py,pz,fx,fy,fz)*
        ...
        if init:  # the walking parameters refer to the last parameters in effect (after a reset, they are pointless)
            ...
            self.obs[
                66] = self.step_obj.step_generator.max_leg_extension / self.step_obj.leg_length  # relative extension of support leg
    ...
    def reward(self, robot) -> float:
        ...
        if robot.joints_position[robot.J_LARM_FRONT] * robot.joints_position[robot.J_RARM_FRONT] < 0:
            reward_points += 4 * 6.1395 * (1 - (robot.joints_position[robot.J_LARM_FRONT] / 120 + robot.joints_position[robot.J_RARM_FRONT] / 120))

        # legs are coordinated
        # small to check: or the sum is very small
        if robot.joints_position[robot.J_LLEG_FRONT] * robot.joints_position[robot.J_RLEG_FRONT] < 0:
            reward_points += 4 * 6.1395 * (1 - (robot.joints_position[robot.J_LLEG_FRONT] + robot.joints_position[robot.J_RLEG_FRONT]) / 75)

        # If right arm in front left leg in front <=> + reward
        # and vice versa
        if robot.joints_position[robot.J_LARM_FRONT] * robot.joints_position[robot.J_RLEG_FRONT] < 0:
            reward_points += 4 * 6.1395
        if robot.joints_position[robot.J_RARM_FRONT] * robot.joints_position[robot.J_LLEG_FRONT] < 0:
            reward_points += 4 * 6.1395

        reward_points -= 10/(0.1 + robot.loc_head_position[2])
        reward_points -= 10/(0.1 + robot.loc_torso_position[2])
        reward_points -= robot.loc_torso_inclination / 6.1395

        if(self.lastyh > 0 and (self.lastyh - robot.loc_head_position[1]) == 0):
            reward_points += 100

        # is moving <=> + reward
        # Arms, legs knees and feet should have movement

        reward_points += abs(robot.joints_speed[robot.J_LARM_FRONT])/2 + abs(robot.joints_speed[robot.J_RARM_FRONT])/2
        reward_points += abs(robot.joints_speed[robot.J_LLEG_FRONT])/2 + abs(robot.joints_speed[robot.J_RLEG_FRONT])/2
        reward_points += abs(robot.joints_speed[robot.J_LFOOT_VER])/2 + abs(robot.joints_speed[robot.J_RFOOT_VER])/2
        ...
        if(self.lastx > 0 and self.lastxt > 0):
            if ((robot.loc_head_position[0] - self.lastx) <0 or (robot.loc_torso_position[0] - self.lastxt) < 0):
                reward_points -= 1000
            else:
        ...
        self.lastyh = robot.loc_head_position[1]

        print(robot.cheat_abs_pos)
    ...
    def step(self, action):
        ...
        if self.step_counter == 0:
            '''
            The first time step will change the paramters of the next footstep
            ...
            '''
            r.behavior.execute("Step", self.step_default_dur, self.step_default_z_span, self.step_default_z_max)
        else:
            ...
            r.behavior.execute("Step", self.step_default_dur, step_zsp, step_zmx)
        ...
        terminal = r.loc_torso_position[2] < 0.2 or self.step_counter > 300 or r.loc_head_position[2] < 0.5
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        def init_env(i_env):
            def thunk():
                return Run_Forward(self.ip, self.server_p + i_env, self.monitor_p_1000 + i_env, self.robot_type, False)
    ...
    def test(self, args):
        ...
        env = Run_Forward(self.ip, self.server_p - 1, self.monitor_p, self.robot_type, True)
