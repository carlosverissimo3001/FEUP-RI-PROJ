...
class Get_Up(gym.Env):
    def __init__(self, ip, server_p, monitor_p, r_type, fall_direction, enable_draw) -> None:
        ...
        self.original_slots = []
        for delta_ms, indices, angles in self.player.world.robot.behavior.slot_engine.behaviors[self.get_up_names[self.fall_direction]]:
    ...
    def get_up(self):
        r = self.player.world.robot
        finished = r.behavior.execute(self.get_up_names[self.fall_direction])
    ...
    def other(self, behavior_name):
        r = self.player.world.robot
        r.behavior.execute(behavior_name)
        self.player.scom.commit_and_send( r.get_command() )
        self.player.scom.receive()
    ...
    def step(self, action):
        ...
        r.behavior.slot_engine.behaviors[self.get_up_names[self.fall_direction]][self.current_slot] = ( new_delta, slice(0,22), angles )
        ...
        if terminal: # network has set values for entire slot, now run behavior and evaluate
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        self.generate_get_up_behavior(model, model_path, folder_name, eval_env.get_attr('original_slots')[0], "last_model.xml")
    ...
    def test(self, args):
        ...
        if not os.path.isfile(os.path.join( args["folder_dir"], XML_name )):
            self.generate_get_up_behavior(model, args["folder_dir"], args["folder_name"], env.original_slots, XML_name )
    ...
    def generate_get_up_behavior(self, model : BaseAlgorithm, folder_dir, folder_name, original_slots, XML_name):
        ...
        self.generate_slot_behavior( folder_dir, folder_name, model, slots, False, XML_name )
