...
class Inv_Kinematics():
    ...
    def _draw_labels(self, player:Agent):
        ...
        for i, body_part in enumerate(['lankle','rankle']):
            ...
            p = r.inv_kinematics.get_body_part_pos_relative_to_hip(body_part) # ankle relative to center of both hip joints
...
Command: {action/option}
    ...
    option: {"",.,g,q,h}
Examples:
    "lx0.1"  - move left ankle to 0.1m in the x-axis
...
------------------------------------------------------------------""")
    ...
    def execute(self):
        ...
        player = Agent(a.i, a.p, a.m, a.u, a.r, a.t, True, True, [])
        ...
        for i in count():
            ...
            if i == next_control_step:
                ...
                if i == next_control_step: break

            for i in range(2): #left and right legs

                indices, values, error_codes = player.world.robot.inv_kinematics.leg(self.feet_pose[i][0], self.feet_pose[i][1], bool(i==0), False)
