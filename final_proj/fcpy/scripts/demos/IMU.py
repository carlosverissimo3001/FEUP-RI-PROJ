...
class IMU():
    ...
    def __init__(self,script:Script) -> None:
        ...
        self.colors = [Draw.Color.white, Draw.Color.red, Draw.Color.green_light]

    def act(self):
        r = self.player.world.robot
        joint_indices = [r.J_LLEG_FRONT, 
                         r.J_LKNEE, 
                         r.J_LFOOT_VER,
                         r.J_LARM_SIDE,
                         r.J_RLEG_FRONT, 
        ...
        self.cycle += 1
        if self.cycle < 50:
            r.set_joints_target_position_direct(joint_indices, np.array([32+10,-64,32, 45,  40+10,-80,40,  0]))        
        elif self.cycle < 100:
            r.set_joints_target_position_direct(joint_indices, np.array([  -10,  0, 0,  0,    -10,  0, 0,  0]))
        elif self.cycle < 150:
            r.set_joints_target_position_direct(joint_indices, np.array([40+10,-80,40,  0,  32+10,-64,32, 45]))
        elif self.cycle < 200:
            r.set_joints_target_position_direct(joint_indices, np.array([  -10,  0, 0,  0,    -10,  0, 0,  0]))
        else:
            self.cycle = 0
    ...
    def act2(self, x, y, rot):
        r = self.player.world.robot
        r.behavior.execute("Walk_ZMP", x, y, rot )
    ...
    def execute(self):
        ...
        self.player = Agent(a.i, a.p, a.m, a.u, 0, a.t, True, True, [])

        self.player.scom.commit_beam((-3,0),15)

        for i in range(10): #beam to place
            self.player.scom.commit_and_send()
            self.player.scom.receive()

        self.player.world.draw.annotation((-3,1,0.9), "IMU + Localizer", self.colors[0], "note_IMU_1", True)
        ...
        self.player.world.draw.annotation((-3,1,0.8), "IMU for rotation", self.colors[1], "note_IMU_2", True)
