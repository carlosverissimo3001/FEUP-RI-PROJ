...
path_ret : numpy array (float32)
    ...
    path_ret[-2]
        ...
        3 - no obstacles between start and target (path_ret[:-1] contains only 2 points: the start and target)
    path_ret[-1]
        A* path cost
::::::::::::::::::::::::::::::::::::::::::
::::::::::::::::::Notes:::::::::::::::::::
...
Map of field:
    ...
    - The path may go outside the field (out of bounds) if the user allows it, 
      but it may never go over goal posts or the goal net (these are considered static inaccessible obstacles)
...
Repulsive force:
    ...
    Example:
        ...
        ..1..   ..o..   .o1..
        ..o..   ..o..   ..o..
    Consider the map on the left where 'x' is the target, 'o' is the player, and '1' is a repulsive force of 1
    On the second map, the player chooses to go forward,      the total cost of this path is: 1+(extra=1)+1   = 3
    On the third  map, the player avoids the repulsive force, the total cost of this path is: sqrt(2)+sqrt(2) = 2.83 (optimal solution)
        ...x...   ..oo...   ...o...   ...o...
        ..123..   .o123..   ..o23..   ..1o3..
        ...o...   ..oo...   ...o...   ...o...
    Consider a new example on the left map. There are 3 position with repulsive forces, going from 1 to 3.
    On the second map, the player avoids all repulsive forces,               total cost: 1+sqrt(2)+sqrt(2)+1       = 4.83
    On the third  map, the player goes through the smallest repulsive force, total cost: sqrt(2)+(extra=1)+sqrt(2) = 3.83 (optimal solution)
    On the fourth map, the player chooses to go forward,                     total cost: 1+(extra=2)+1             = 4.00
...
class Pathfinding():
        ...
        for x in np.arange(-16,16.01,0.1):
            for y in np.arange(-11,11.01,0.1):
                ...
                if s_out != 3:
                    ...
                    d.point((x,y), 4, d.Color.get(255,165,0), "grid", False)
                elif cost_in > 0.11:
                    d.point((x,y), 4, d.Color.get(255,210,0), "grid", False)
                elif cost_in > 0.01:
                    d.point((x,y), 4, d.Color.get(255,255,0), "grid", False)
    ...
    def execute(self):
        ...
        while True:
            ball = w.ball_cheat_abs_pos[:2]
            rpos = r.cheat_abs_pos[:2]
