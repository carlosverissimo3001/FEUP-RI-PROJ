...
class Train_Base():
    ...
    def prompt_user_for_model():
        ...
        folders.insert(0,"(Abort)")

        idx, folder_name = UI.print_list(folders,prompt="Choose folder: ")

        if idx == 0: # user aborted
            return None
        ...
        if not models:
            ...
            return Train_Base.prompt_user_for_model()

        models.sort(key=lambda m: os.path.getmtime(join(folder_dir, m+".zip")), reverse=True) # sort by modification date
        models.insert(0,"(Abort)")
        
        idx, model_name = UI.print_list(models,prompt="Choose model: ")
    ...
    def generate_slot_behavior(self, path, model_name, model:BaseAlgorithm, slots, auto_head:bool, XML_name):
        '''
        ...
        now = datetime.now()
        xml_root_name = now.strftime( f"{model_name}__%d_%m_%Y__%H_%M_%S" )

        # create the file structure
        auto_head = '1' if auto_head else '0'
        EL_behavior = ET.Element('behavior',{'name':xml_root_name,'type':'SlotBehavior','description':'Add description to XML file', "auto_head":auto_head})
