...
class Run_Forward(gym.Env):
    ...
    def reward(self, robot) -> float:
        ...
        if robot.joints_position[6] * robot.joints_position[7] < 0:
            reward_points += 4 * (1 - (robot.joints_position[6] + robot.joints_position[7]) / 75)
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        def init_env(i_env):
            def thunk():
                return Run_Forward(self.ip, self.server_p + i_env, self.monitor_p_1000 + i_env, self.robot_type, True)
